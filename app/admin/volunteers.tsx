...
export default function VolunteersScreen() {
  ...
  const addMutation = useMutation({
    mutationFn: async (params: { name: string; phone: string }) => {
      ...
      // In a real implementation, createVolunteer would accept the pubkey
      // For now, we return the nsec for the admin to share
    ...
    onError: () => Alert.alert(t('admin.error', 'Error'), t('admin.addError', 'Failed to add volunteer')),
  })

  const inviteMutation = useMutation({
    mutationFn: async () => {
      // createInvite would be an API call
      return { code: 'mock-invite-code' }
    },
    onSuccess: async (result) => {
      const inviteUrl = `llamenos://invite?code=${result.code}`
      await Share.share({
      ...
      {generatedNsec && (
        <View className="mx-4 mt-4 rounded-xl border border-yellow-500/30 bg-yellow-500/10 p-4">
          <Text className="mb-2 text-sm font-semibold text-yellow-700">
            {t('admin.nsecWarning', 'Share this key with the volunteer. It will only be shown once.')}
          </Text>
          ...
          >
            <Text className="text-xs text-yellow-700">{t('common.dismiss', 'Dismiss')}</Text>
          </Pressable>
        </View>
      )}
        ...
        {canCreate && (
          ...
          >
            <Text className="text-center text-sm font-semibold text-primary-foreground">
              {t('admin.addVolunteer', 'Add Volunteer')}
            </Text>
          </Pressable>
        )}
        {canInvite && (
